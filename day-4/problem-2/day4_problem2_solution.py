class Solution(object):
    def get_number_of_positive_integers(self, nums: list[int]) -> int:
        low = 0
        high = len(nums) - 1
        num_pos_integers = 0
        found = False
    ...
    def get_number_of_negative_integers(self, nums: list[int]) -> int:
        low = 0
        high = len(nums) - 1
        num_neg_integers = 0
        found = False
    ...
    def maximumCount(self, nums: list[int]) -> int:
        ...
        if (nums[low] > 0 and nums[high] > 0) or (nums[low] < 0 and nums[high] < 0):
            ...
            num_positive_integers = self.get_number_of_positive_integers(nums)
            num_negative_integers = self.get_number_of_negative_integers(nums)
